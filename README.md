# EcoStep Carbon Calculator & Dashboard
## INTRODUCTION

The EcoStep Carbon Calculator is a Python-based web application that was created to help
users understand and track their daily carbon footprint in a simple, interactive way. It focuses
on three main lifestyle areas, including transportation, household electricity use, and diet. It
helps give a clear picture of how everyday choices have a big impact on the environment.
The app calculates emissions in standard units like kg CO₂e, and convert those numbers into
more relatable comparisons, such as how many trees would be needed to offset it, how many
smartphone charges it equals, or how many kilometers can be driven in a petrol car.

## OBJECTIVES

• To provide the users with an easy tool to estimate their daily and monthly carbon
emissions.
• To increase awareness of how transport, electricity use, and diet contribute to an
individual’s footprint.
• To translate abstract emission numbers into relatable equivalents that motivate behaviour
change.
• To offer simple, personalized tips that help users reduce their overall emissions.

## KEY FEATURES

• Feature 1: Transport Calculator
Calculates CO₂ based on fuel type and travel
distance.

• Feature 2: Electricity Usage Calculator
Shows emissions from daily household electricity
use.

• Feature 3: Diet Impact Estimator
Estimates emissions based on your diet (meat,
mixed, vegetarian/vegan).

## EXTRA FEATURES

• Daily and monthly summaries
• Impact levels (Low, Moderate, High)
• Relatable equivalents (like trees, km driven, phone charges)
• Charts and dashboards for easy understanding

### 1. Main dashboard interface

<img width="655" height="342" alt="image" src="https://github.com/user-attachments/assets/7db8bd5c-1089-41c7-9993-40ec56e31fa4" />

### 2. Sidebar

<img width="507" height="485" alt="image" src="https://github.com/user-attachments/assets/75c69177-9268-43a5-84b0-3a95d1513ed6" />

<img width="521" height="413" alt="image" src="https://github.com/user-attachments/assets/c79fb115-c5de-4199-abb8-d175e1936b61" 

### 3. Data Input Section

<img width="534" height="435" alt="image" src="https://github.com/user-attachments/assets/123e7b27-6946-4c88-8424-15687c30e72b" />


A. Transportation

<img width="476" height="531" alt="image" src="https://github.com/user-attachments/assets/b3c05a56-ac4b-4dff-ac33-791648e0095e" />

B. Electricity Usage

<img width="496" height="452" alt="image" src="https://github.com/user-attachments/assets/49c7517b-2af5-4f79-a82a-6b5a6a9acc04" />

C. Diet

<img width="476" height="430" alt="image" src="https://github.com/user-attachments/assets/1846d9c6-8be7-4842-b699-0a164c50f49f" />

### 4. Result Section

<img width="640" height="409" alt="image" src="https://github.com/user-attachments/assets/78421e25-cecd-4688-a6d0-e6f728da4794" />

<img width="389" height="479" alt="image" src="https://github.com/user-attachments/assets/9920dd02-3efc-44f1-8ed7-94a49a2a06fe" />

### 5. Visualization Section

<img width="634" height="317" alt="image" src="https://github.com/user-attachments/assets/8ed11c18-fcf9-4e01-9449-d2e37b7ce176" />

### 6. Smart Suggestion Section 

<img width="638" height="327" alt="image" src="https://github.com/user-attachments/assets/b4d9fc35-d352-483e-b69e-36538a51c130" />

### 7. Export Section

<img width="659" height="190" alt="image" src="https://github.com/user-attachments/assets/e77ebb44-6437-4a63-b5b9-2491c686a6a6" />

## RESOURCES AND TOOLS USED

• Programming language: Python.
• Framework for UI: Streamlit for creating the interactive web dashboard.
• Libraries:
- Pandas for data handling and calculations.
- Matplotlib for charts and visual summaries.
- datetime for handling dates and timestamps.
• Data/emission factors:
- Transport and electricity emission:
▪ https://www.bajajfinserv.in/what-is-the-power-consumption-of-1-5-ton-ac
▪ https://smarterbusiness.co.uk/blogs/how-much-energy-do-my-appliances-useinfographic/
▪ https://www.gov.uk/government/publications/greenhouse-gas-reportingconversion-factors-2025
- Diet-related emission:
▪ https://ourworldindata.org/environmental-impacts-of-food
• Development environment: VS Code













